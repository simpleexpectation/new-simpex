...
const scoreTopicByState = (topic: any) => {
...
const buildRecommendedTopics = (topics: any[]) => [...topics]
  .sort((left, right) => {
...
const isHorizontalSwipe = (startX: number, startY: number, endX: number, endY: number) => {
...
Page({
  ...
  onTopicSectionChange(e: WechatMiniprogram.SwiperChange) {
  ...
  startPageSwipe(e: WechatMiniprogram.TouchEvent) {
    const touch = e.touches[0]
    if (!touch) return
    ;(this as any).pageSwipeStart = {
      x: touch.clientX,
      y: touch.clientY
    }
  },
  endPageSwipe(e: WechatMiniprogram.TouchEvent) {
    const start = (this as any).pageSwipeStart
    const touch = e.changedTouches[0]
    ;(this as any).pageSwipeStart = null
  ...
  onRealitySwiperChange(e: WechatMiniprogram.SwiperChange) {
